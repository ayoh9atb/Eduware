...
public class ForumBean {
    ...
    private static final String FORUM_ID_KEY = "online_forum_id";
    //
    private static final String FORUM_ID_DEFAULT = "1";
    //
    private ForumEntity forum;
    private List<ForumEntity> forumList;
    private List<SelectItem> forumItemList;

    public ForumBean() {
        forum = new ForumEntity();
        forumList = new ArrayList<ForumEntity>();
        forumItemList = new ArrayList<SelectItem>();
    ...
    public void loadForumList() {
        ...
        ForumEntity criteria = new ForumEntity();

        try {
            List<AbstractEntity> abstractFacultyList = dataServer.findData(criteria);
            for (AbstractEntity entity : abstractFacultyList) {
                ForumEntity forumEntity = (ForumEntity) entity;

                forumEntity.setCreateDateTO(DateUtil.toUtilDate(forumEntity.getCreateDate()));

                forumList.add(forumEntity);
                addToSelectItemList(forumEntity);
            }
        } catch (Exception ex) {
            ex.printStackTrace();
    ...
    public void addToSelectItemList(ForumEntity entity) {
        ...
        if (CommonBean.USER_ADMIN_SWITCH == UserAdminSwitchType.ON) {
            ...
            if (entity.getApprovalStatusID() >= privilege.getApprovedStatusID()) {
                ...
                item.setValue(entity.getForumID());
                item.setLabel(entity.getForumName());
                forumItemList.add(item);
            }
    ...
    public String addForum() {
        if (!getForumList().contains(getForum())) {
            getForum().setCreateDateTO(new Date());
            getForumList().add(getForum());
        } else {
            int index = getForumList().indexOf(getForum());
            getForumList().set(index, getForum());
        }
        setForum(new ForumEntity());

        return "";
    }

    public String saveForumList() {
        FinultimatePersistenceRemote dataServer = FinanceServiceLocator.locateFinancePersistenceManager();
        ...
        try {
            for (ForumEntity forum : getForumList()) {
                forum.setCreateDate(DateUtil.toSQLDate(forum.getCreateDateTO()));
                if (forum.getForumID() == 0) {
                    try {
                        int cmID = Integer.parseInt(appPropBean.getValue(FORUM_ID_KEY,
                                FORUM_ID_DEFAULT, true));
                        forum.setForumID(cmID);
                        dataServer.saveData(forum);
                    } catch (Exception ex) {
                        ex.printStackTrace();
                    }
                } else if (forum.getForumID() > 0) {
    ...
    public String activate() {
        ...
        if (forum.getApprovalStatusID() < privilege.getApprovedStatusID()) {
            if (userManagerBean.getUserAccount().getRole().getApprovalLevelID() > 0) {
                if ((forum.getApprovalStatusID() == 0) || (forum.getApprovalStatusID()
                        <= userManagerBean.getUserAccount().getRole().getApprovalLevelID())) {
                    UserAccountEntity creatorAcct = userManagerBean.loadUserAccount(forum.getApprover());
                    if (userManagerBean.getUserAccount().getRole().getApprovalLevelID()
                            > creatorAcct.getRole().getApprovalLevelID()) {
                        forum.setApprovalStatusID(forum.getApprovalStatusID() + 1);
                        forum.setApprover(userManagerBean.getUserAccount().getUserName());
                ...
                } else if (forum.getApprovalStatusID()
                        > userManagerBean.getUserAccount().getRole().getApprovalLevelID()) {
    ...
    public String loadClassOfDegreeList(BusinessActionTrailEntity businessActionTrail) throws Exception {
        ...
        for (AbstractEntity entity : entityList) {
            ForumEntity forum = (ForumEntity) entity;
            forumList.add(forum);
        }
        ...
        try {
            if (forumList.size() > 0) {
                BusinessActionTrailEntity batEntity = businessActionTrailBean.loadPreviousHistoricalState(
                        forumList.get(0).getActionTrail());
                if (batEntity != null) {
        ...
        try {
            if (forumList.size() > 0) {
                BusinessActionTrailEntity batEntity = businessActionTrailBean
                        .loadNextHistoricalState(forumList.get(0).getActionTrail());
                if (batEntity != null) {
                    loadClassOfDegreeList(batEntity);
                } else {
    ...
    public String gotoAuditTrailPage() {
        ...
        businessActionTrailBean.getBusinessActionTrail().setEntityNameType(EntityNameType.FORUM);
        businessActionTrailBean.getBusinessActionTrail().setEntityID(forum.getForumID());
    ...
    public String deleteClassOfDegree() {
        ...
        try {
            if (forumList.contains(forum)) {
                if (forum.getForumID() > 0) {
                    ForumEntity criteria = new ForumEntity();
                    criteria.setForumID(forum.getForumID());
    ...
    public ForumEntity findForumByID(long forumID) {
        ForumEntity forum = null;

        for (ForumEntity forumEntity : getForumList()) {
            if (forumEntity.getForumID() == forumID) {
                forum = forumEntity;
                break;
            }
        }
    ...
    public void forumSelected(ValueChangeEvent vce) {
        int rowIndex = CommonBean.getCheckBoxEventRowIndex(vce);
        setForum(getForumList().get(rowIndex));
    }
    ...
    public List<SelectItem> getForumItemList() {
        return this.forumItemList;
    }

    public void setForumItemList(List<SelectItem> forumItemList) {
