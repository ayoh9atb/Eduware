 ...
 */
@ApplicationScoped
@ManagedBean(name = "schoolTierTypeBean")
public class SchoolTierTypeBean {

    private static final String SCHOOL_TIER_ID_KEY = "schl_tier_id";
    //
    private static final String SCHOOL_TIER_ID_DEFAULT = "1";
    //
    private SchoolTierTypeEntity schoolTierType;
    private List<SchoolTierTypeEntity> schoolTierTypeList;
    private List<SelectItem> schoolTierTypeItemList;

    public SchoolTierTypeBean() {
        schoolTierType = new SchoolTierTypeEntity();
        schoolTierTypeList = new ArrayList<SchoolTierTypeEntity>();
        schoolTierTypeItemList = new ArrayList<SelectItem>();

        loadSchoolTier();
    }

    public void loadSchoolTier() {
        ...
        SchoolTierTypeEntity criteria = new SchoolTierTypeEntity();

        try {
            List<AbstractEntity> abstractSchoolTierList = dataServer.findData(criteria);
            for (AbstractEntity entity : abstractSchoolTierList) {
                SchoolTierTypeEntity tierEntity = (SchoolTierTypeEntity) entity;
                getSchoolTierTypeList().add(tierEntity);
    ...
    public void addToSelectItemList(SchoolTierTypeEntity entity) {
        ...
        if (CommonBean.USER_ADMIN_SWITCH == UserAdminSwitchType.ON) {
            ...
            if (entity.getApprovalStatusID() >= privilege.getApprovedStatusID()) {
                ...
                item.setValue(entity.getSchoolTierID());
                item.setLabel(entity.getSchoolTierName());
                schoolTierTypeItemList.add(item);
            }
    ...
    public String addSchoolTierType() {
        if (!getSchoolTierTypeList().contains(getSchoolTierType())) {
            getSchoolTierTypeList().add(getSchoolTierType());
            setSchoolTierType(new SchoolTierTypeEntity());
        }

        return "";
    }

    public String saveSchoolTierTypeList() {
        FinultimatePersistenceRemote dataServer = FinanceServiceLocator.locateFinancePersistenceManager();
        ...
        try {
            for (SchoolTierTypeEntity tier : getSchoolTierTypeList()) {
                if (tier.getSchoolTierID() == 0) {
                    try {
                        int cmID = Integer.parseInt(appPropBean.getValue(SCHOOL_TIER_ID_KEY,
                                SCHOOL_TIER_ID_DEFAULT, true));
                        tier.setSchoolTierID(cmID);
                        dataServer.saveData(tier);
                    } catch (Exception ex) {
                        ex.printStackTrace();
                    }
                } else if (tier.getSchoolTierID() > 0) {
                    dataServer.updateData(tier);
                }
            }
    ...
    public String activate() {
        ...
        if (schoolTierType.getApprovalStatusID() < privilege.getApprovedStatusID()) {
            if (userManagerBean.getUserAccount().getRole().getApprovalLevelID() > 0) {
                if ((schoolTierType.getApprovalStatusID() == 0) || (schoolTierType.getApprovalStatusID()
                        <= userManagerBean.getUserAccount().getRole().getApprovalLevelID())) {
                    UserAccountEntity creatorAcct = userManagerBean.loadUserAccount(schoolTierType.getApprover());
                    if (userManagerBean.getUserAccount().getRole().getApprovalLevelID()
                            > creatorAcct.getRole().getApprovalLevelID()) {
                        schoolTierType.setApprovalStatusID(schoolTierType.getApprovalStatusID() + 1);
                        schoolTierType.setApprover(userManagerBean.getUserAccount().getUserName());

                        dataServer.beginTransaction();
                        try {
                            dataServer.updateData(schoolTierType);
                            dataServer.endTransaction();
                        } catch (SQLException ex) {
                            dataServer.rollbackTransaction();
                            applicationMessageBean.insertMessage(ex.getMessage(), MessageType.ERROR_MESSAGE);
                ...
                } else if (schoolTierType.getApprovalStatusID()
                        > userManagerBean.getUserAccount().getRole().getApprovalLevelID()) {
    ...
    public String loadSchoolTierList(BusinessActionTrailEntity businessActionTrail) throws Exception {
        schoolTierType = new SchoolTierTypeEntity();
        schoolTierTypeList = new ArrayList<SchoolTierTypeEntity>();
        schoolTierTypeItemList = new ArrayList<SelectItem>();
        ...
        for (AbstractEntity entity : entityList) {
            SchoolTierTypeEntity schoolTierType = (SchoolTierTypeEntity) entity;
            schoolTierTypeList.add(schoolTierType);
        }
    ...
    public String loadPreviousHistoricalState() {
        ...
        try {
            if (schoolTierTypeList.size() > 0) {
                BusinessActionTrailEntity batEntity = businessActionTrailBean.loadPreviousHistoricalState(
                        schoolTierTypeList.get(0).getActionTrail());
                if (batEntity != null) {
                    loadSchoolTierList(batEntity);
                } else {
        ...
        try {
            if (schoolTierTypeList.size() > 0) {
                BusinessActionTrailEntity batEntity = businessActionTrailBean
                        .loadNextHistoricalState(schoolTierTypeList.get(0).getActionTrail());
                if (batEntity != null) {
                    loadSchoolTierList(batEntity);
                } else {
    ...
    public String gotoAuditTrailPage() {
        ...
        businessActionTrailBean.getBusinessActionTrail().setEntityNameType(EntityNameType.SCHOOL_TIER_TYPE);
        businessActionTrailBean.getBusinessActionTrail().setEntityID(schoolTierType.getSchoolTierID());
        ...
        try {
            if (businessActionTrailBean.getBusinessActionTrailList().size() > 0) {
                ...
                loadSchoolTierList((BusinessActionTrailEntity) businessActionTrailBean.getBusinessActionTrail().cloneEntity());
                outcome = "schooltiertypeaudittrail.jsf";
            } else {
    ...
    public String deleteSchoolTierType() {
        ApplicationMessageBean applicationMessageBean = (ApplicationMessageBean) CommonBean.getBeanFromContext(
                "#{sessionScope.applicationMessageBean}", ApplicationMessageBean.class);
        if (applicationMessageBean == null) {
            applicationMessageBean = new ApplicationMessageBean();
        ...
        dataServer.beginTransaction();
        try {
            if (schoolTierTypeList.contains(schoolTierType)) {
                if (schoolTierType.getSchoolTierID() > 0) {
                    SchoolTierTypeEntity criteria = new SchoolTierTypeEntity();
                    criteria.setSchoolTierID(schoolTierType.getSchoolTierID());

                    dataServer.deleteData(criteria);
                    dataServer.endTransaction();
                }

                schoolTierTypeList.remove(schoolTierType);
                schoolTierType = new SchoolTierTypeEntity();
            }
        } catch (SQLException ex) {
            dataServer.rollbackTransaction();
    ...
    public SchoolTierTypeEntity findSchoolTierByID(int tierID) {
        SchoolTierTypeEntity tier = null;

        for (SchoolTierTypeEntity schoolTier : getSchoolTierTypeList()) {
            if (schoolTier.getSchoolTierID() == tierID) {
                tier = schoolTier;
                break;
            }
        }
    ...
    public void schoolTierTypeItemSelected(ValueChangeEvent vce) {
    }

    public void schoolTierTypeSelected(ValueChangeEvent vce) {
        int rowIndex = CommonBean.getCheckBoxEventRowIndex(vce);
    }

    public SchoolTierTypeEntity getSchoolTierType() {
        return this.schoolTierType;
    }

    public void setSchoolTierType(SchoolTierTypeEntity schoolTierType) {
        this.schoolTierType = schoolTierType;
    }

    public List<SchoolTierTypeEntity> getSchoolTierTypeList() {
        return this.schoolTierTypeList;
    }

    public void setSchoolTierTypeList(List<SchoolTierTypeEntity> schoolTierTypeList) {
        this.schoolTierTypeList = schoolTierTypeList;
    }

    public List<SelectItem> getSchoolTierTypeItemList() {
        return this.schoolTierTypeItemList;
    }

    public void setSchoolTierTypeItemList(List<SelectItem> schoolTierTypeItemList) {
        this.schoolTierTypeItemList = schoolTierTypeItemList;
    }
}
